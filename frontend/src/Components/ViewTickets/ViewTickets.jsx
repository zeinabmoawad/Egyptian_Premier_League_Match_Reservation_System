    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:8000/api/v1/reservation/user_reservations",
          {
            headers: {
  ...
  const handleBackendCancellation = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `http://localhost:8000/api/v1/reservation/delete_reservation/${id}`,
        {
          headers: {
            Authorization: `Bearer ${storedToken}`,
            "Content-Type": "application/json", // Adjust content type as needed
      ...
      <Container className="matches-container">
        ...
        <Row className="matches justify-content-start">
          {matches.map((item, i) => (
            <Col md={3} key={i}>
              <div className="match">
                {isDateWithinThreeDays(item.matchId.time) ? (
                  <FcCancel
                ...
                <div className="match-teams">
                  ...
                  <div className="match-day-time">
                    <p className="match-time">
                      {new Date(item.matchId.time).getHours() < 10 ? "0" : ""}
                      {new Date(item.matchId.time).getHours()}:
                      {new Date(item.matchId.time).getMinutes() < 10 ? "0" : ""}
                      {new Date(item.matchId.time).getMinutes()}
                    </p>
                    <p className="match-day">
                      {new Date(item.matchId.time).getMonth() + 1 < 10
                        ? "0"
                        : ""}
                      {new Date(item.matchId.time).getMonth() + 1}/
                      {new Date(item.matchId.time).getMonth() < 10 ? "0" : ""}
                      {new Date(item.matchId.time).getDate()}/
                      {new Date(item.matchId.time).getFullYear()}
                    </p>
                  </div>
